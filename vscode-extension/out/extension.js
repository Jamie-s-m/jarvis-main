...
function activate(context) {
    ...
    const startCmd = vscode.commands.registerCommand('jarvis.startEngine', async () => {
        ...
        const candidates = [];
        if (extRoot)
            candidates.push(path.join(extRoot, 'dist', 'JarvisAgent.exe'));
        ...
        if (wf)
            candidates.push(path.join(wf, 'dist', 'JarvisAgent.exe'));
        ...
        try {
            ...
            if (exePath) {
                ...
                if (wf && fs.existsSync(path.join(wf, 'jarvis_desktop.py')))
                    script = path.join(wf, 'jarvis_desktop.py');
    ...
    const showMenuCmd = vscode.commands.registerCommand('jarvis.showEngineMenu', async () => {
        ...
        if (pick === 'View HUD') {
            try {
                await vscode.commands.executeCommand('workbench.view.extension.jarvisHud');
            }
            catch (e) { /* best-effort */ }
        }
        if (pick === 'Stop Engine') {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('jarvisHud', provider));
